...
- Every aggregate and projection detail page carries a **Json** tab beside **State**, showing the stored payload as the store holds it: laid out one value per line, coloured by kind of token, capped in height so a large model scrolls inside its box, with a **Copy** button that puts the laid-out text on the clipboard. It is the row's own text rather than the model serialised again, so a payload the model cannot read back — or one carrying more than the model declares — is still there to see, and one that is not JSON at all is shown as it is under a note saying why. See [Memoria Web](tools/memoria-web.md#what-it-shows)
